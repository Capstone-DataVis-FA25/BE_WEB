...
export class ChartsService {
  ...
  async create(createChartDto: CreateChartDto, userId: string) {
    ...
    // Validate that config exists and is not empty
    if (!config || typeof config !== "object") {
      ...
      throw new BadRequestException(
        "Chart config is required and must be a valid object"
      );
    }

    console.log("✅ Creating chart with valid config:", {
      name,
      type,
      datasetId,
    ...
    // Ensure config is serializable JSON
    let jsonConfig: any;
    try {
      jsonConfig = JSON.parse(JSON.stringify(config));
    } catch (error) {
      console.error("❌ Config serialization failed:", error);
      throw new BadRequestException(
        "Chart config must be valid JSON-serializable object"
      );
    }
